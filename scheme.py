...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        result = procedure.env.make_call_frame(procedure.formals, args)
        return scheme_eval(procedure.body,result)
    elif isinstance(procedure, MuProcedure):
        "*** YOUR CODE HERE ***"
        check_formals(procedure.formals)
        result = env.make_call_frame(procedure.formals, args)
        return scheme_eval(procedure.body,result)
    else:
...
class Frame(object):
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(vals) != len(formals):
            raise SchemeError('Number of parameters and values do not match')
        else:
            while formals is not nil and scheme_pairp(formals):
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        body = Pair(vals.first.second,vals.second)
        env.define(vals.first.first, do_lambda_form(body, env))
        return target.first
    else:
...
def check_formals(formals):
    ...
    try:
        for f in formals:
            if not scheme_symbolp(f) or scheme_numberp(f):
                raise SchemeError('Formal is not a symbol.')
            if f in visited:
                raise SchemeError('Symbol can not be repeated.')
            else:
                visited.append(f)
    except TypeError:
        SchemeError('Valid parameter list was not given.')
